...
function buildCoachReply(text) {
  const pick = (items) => items[(text.length + records.length) % items.length];
  if (/圖|格|面積|看不出/.test(text)) {
    return pick([
      "你願意說出「看圖看不出來」其實很棒，這代表你不是亂猜答案。先做一小步就好：先找全部被分成幾份，再找被選到幾份。",
      "這比較像是圖還沒跟算式接起來，不是你不會。你先不用算，先告訴我：圖上全部大概被切成幾份？",
      "看圖卡住很正常，我們先不要碰公式。只做第一步：找整體，再找被選到的部分。"
    ]);
  }
  if (/分母/.test(text)) {
    return pick([
      "你注意到分母，這是很好的問題。分母是在說整體被切成幾份。先想：如果每一份又再切小，全部份數會不會變多？",
      "你不是亂問，分母真的很關鍵。先不用背公式，我們只看「切法」有沒有變細。",
      "分母可以先想成盤子被切幾塊。乘法裡常常是切了又切，所以份數會重新整理。"
    ]);
  }
  if (/分子/.test(text)) {
...
function buildComfortReply(text) {
  const talkCount = diagnosticMessages.querySelectorAll(".diagnostic-message.student").length;
  const pick = (items) => items[(text.length + talkCount + records.length) % items.length];
  if (/不說|算了|不要說|不想講|懶得說/.test(text)) {
    return pick([
      "可以，不想說也沒關係。我不會逼你。那我先陪你安靜一下，你只要回一個表情或一個字也可以。",
      "好，那我們先不追問。你可以把這裡當成休息一下的地方，等你想講一點點再講。",
      "收到。你現在不想說，我尊重你。那我先問很小的一題：你比較想安靜、看小影片，還是我陪你待一下？"
    ]);
  }
  if (/笨|爛|廢|白癡|沒用/.test(text)) {
    return pick([
      "我先把這句接住：你不是笨。你可能是累了、挫折了，或被某件事卡住了。要不要先告訴我，是誰讓你有這種感覺？",
      "聽起來你現在很討厭自己，這種感覺很重。先不用證明你很好，我會陪你把這團感覺放小一點。",
      "你可以罵自己，但我不會跟著罵你。你現在比較像是受挫，不是沒用。剛剛發生了什麼？"
    ]);
  }
  if (/幹嘛|干嘛|怎樣|煩欸|吵|閉嘴/.test(text)) {
    return pick([
      "我知道，你現在可能不想被問太多。那我放輕一點：你只要選一個就好，想安靜、想吐槽、還是想看小影片？",
      "好，我先退一步，不碎念。你現在比較需要有人聽你抱怨，還是需要先不要講話？",
      "可以，我不急著拉你去學習。你現在火氣或煩躁感比較多嗎？我先聽你說。"
    ]);
  }
  if (/哭|難過|傷心|委屈|想哭/.test(text)) {
    return pick([
      "聽起來你今天真的很難受。你不用急著把自己整理好，我會先陪你。是有人讓你不舒服，還是今天發生了什麼事？",
      "這聽起來不是小事。你可以慢慢說，不用說得很完整。我先陪你把心情放下來一點。",
      "如果你現在很委屈，我會先站在你這邊聽。你想從哪一段開始講都可以。"
    ]);
  }
  if (/生氣|氣|火大|討厭/.test(text)) {
    return pick([
      "感覺你心裡有一股氣卡住了。先不用壓下去，也不用假裝沒事。你想罵一罵也可以，我會聽。",
      "你現在像是很不爽。可以先不用講道理，先講發生什麼事就好。",
      "討厭的感覺通常有原因。是有人講了什麼，還是事情一直不順？"
    ]);
  }
  if (/累|好累|沒力|壓力|煩|不想/.test(text)) {
    return pick([
      "我聽到了，你現在可能真的很累。累的時候什麼事都會變得更重。你比較像身體累，還是心裡累？",
      "如果今天已經很累，我們先不要把事情加重。你可以只回我：累、煩、還是不想動。",
      "煩的時候被一直問也會更煩。我先簡單陪你：要不要先看一段小影片，或只聊兩句？"
    ]);
  }
  ...
  return pick([
    "謝謝你願意跟我說。這裡不用說得很完整，也不用說得很漂亮。我先聽你講。你想從今天發生什麼事開始說，還是只想先說現在的感覺？",
    "我有聽到你在丟出一點點訊號。沒關係，我們慢慢來。你可以再補一句：現在比較煩、累、生氣，還是不知道怎麼說？",
    "先不用把話講清楚。你願意打出來就已經算有開始了。我陪你接著拆一點點：這件事是跟人有關，還是跟今天的事情太多有關？",
    "我不會急著判斷你。你可以講很短也可以。現在你最想要的是被聽見、被安慰，還是先轉移一下注意力？"
  ]);
}
...
function selectMood(moodKey) {
  ...
  if (moodKey === "unhappy") {
    ...
    currentRecommendationAction = { type: "video", url: relaxVideo.url, title: relaxVideo.title, note: "先放鬆一下。超過 10 分鐘，我會再輕輕提醒你下一步。" };
...
function selectDiagnosticUnit(unitKey) {
  ...
  recommendReason.textContent = "還沒選困難類型也沒關係，可以先從影片學習開始。";
  recommendTitle.textContent = unit.firstStep;
  recommendDetail.textContent = `${mood.mode}：先看一小段影片，再依照你的回答調整練習。`;
  setRecommendationAction("concept", unitKey, selectedMood === "unhappy");
  startRecommendationBtn.disabled = false;
  unitReplies.classList.add("is-hidden");
